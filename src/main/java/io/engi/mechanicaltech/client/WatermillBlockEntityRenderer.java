...
public class WatermillBlockEntityRenderer extends BlockEntityRenderer<WatermillBlockEntity> {
    public WatermillBlockEntityRenderer(BlockEntityRenderDispatcher dispatcher) {
        super(dispatcher);
    }

    @Override
    public void render(WatermillBlockEntity blockEntity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		...
		BakedModel model = MinecraftClient.getInstance().getBakedModelManager().getModel(
			new ModelIdentifier(
				new Identifier(MODID, "waterwheel"),
				"facing=" + blockState.get(HorizontalFacingBlock.FACING).asString().toLowerCase() +
				",water_level=" + blockState.get(WaterwheelBlock.WATER_LEVEL)
			)
		);
